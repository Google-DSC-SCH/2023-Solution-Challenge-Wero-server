...
import com.gdsc.wero.global.auth.oauth.dto.OAuthAttributes;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    private User saveOrUpdate(OAuthAttributes attributes){
        User user = userRepository.findUserByEmailAndProvider(attributes.getEmail(), attributes.getProvider())
                .map(entity -> entity.update(attributes.getName(), attributes.getEmail(), attributes.getPicture(), attributes.getProvider()))
                .orElse(attributes.getEntity());
